...
public class ReportServiceImpl implements ReportService {
    ...
    public String generateSlaComplianceReport(String format) throws DAOException {
        ...
        sb.append("=================================================================================\n");
        sb.append(String.format("%-15s %-12s %-15s %-20s %-15s\n", "TICKET #", "PRIORITY", "STATUS", "DEADLINE", "SLA STATUS"));
        sb.append("---------------------------------------------------------------------------------\n");

        for (TroubleTicket t : tickets) {
            String slaStatusStr = "WITHIN_SLA";
            if (t.getStatus() == TicketStatus.RESOLVED || t.getStatus() == TicketStatus.CLOSED) {
                if (t.getResolutionDate() != null && t.getSlaDeadline() != null && t.getResolutionDate().isAfter(t.getSlaDeadline())) {
                    slaStatusStr = "BREACHED";
                } else {
                    slaStatusStr = "COMPLIANT";
                }
            } else if (t.getSlaDeadline() != null) {
                if (DateUtil.isDeadlineExceeded(t.getSlaDeadline())) {
                    slaStatusStr = "BREACHED";
                } else if (DateUtil.isAtRisk(t.getSlaDeadline())) {
                    slaStatusStr = "AT_RISK";
                }
            }

            String deadlineStr = t.getSlaDeadline() != null ? t.getSlaDeadline().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")) : "N/A";
            sb.append(String.format("%-15s %-12s %-15s %-20s %-15s\n",
        ...
        if ("CSV".equalsIgnoreCase(format) || "TXT".equalsIgnoreCase(format)) {
            exportReportToFile("sla_compliance_report", format, resultStr);
        }
    ...
    public String generateEngineerPerformanceReport(String format) throws DAOException {
        ...
        if ("CSV".equalsIgnoreCase(format) || "TXT".equalsIgnoreCase(format)) {
            exportReportToFile("engineer_performance_report", format, resultStr);
        }
    ...
    public String generateIncidentAnalysisReport(String format) throws DAOException {
        ...
        // Java 8 Stream groupingBy category
        ...
        if ("CSV".equalsIgnoreCase(format) || "TXT".equalsIgnoreCase(format)) {
            exportReportToFile("incident_analysis_report", format, resultStr);
        }
    ...
    private void exportReportToFile(String baseName, String format, String content) {
        ...
        try (FileWriter writer = new FileWriter(filename)) {
            ...
            System.out.println("✓ Report exported to: " + new File(filename).getAbsolutePath());
        } catch (IOException e) {
